...
def notify_on_follow(sender , instance , created , **kwargs):
    if created:
        ...
        count = Notifications.objects.filter(recipient=instance.following , is_read=False).count()
...
def remove_follow_notification(sender , instance , **kwargs):
    ...
    async_to_sync(channel_layers.group_send)(
        ...
        {
            'type': 'send_unfollow_notification',
            'event_type': 'unfollow_notification',
            'notification': f'{recipient_user.username} unfollowed you',
            'unread_count': count
        }
    )
    if deleted:
...
def notify_on_like(sender , instance , action , pk_set ,  **kwargs):
    ...
    if action == 'post_add' :
        for user_id in pk_set :
            ...
            if not exists and instance.author != user :
                ...
                count = Notifications.objects.filter(recipient=instance.author , is_read=False).count()
    ...
    if action == 'post_remove' :
        ...
        recipient_id = instance.author.id
        count = Notifications.objects.filter(recipient=recipient_id , is_read=False).count()
        channel_layer = get_channel_layer()
