...
import { Bell, Heart, MessageCircle, UserPlus, Check, CheckCheck ,  Trash2 } from 'lucide-react';
...
const ShowNotifications = () => {
  ...
  const fetchAllNotifications = async () => {
    try {
      ...
      setCounts({
        comment_count: sampleNotifications.filter(n => n.notification_type === 'comment').length,
        like_count: sampleNotifications.filter(n => n.notification_type === 'like').length,
        follow_count: sampleNotifications.filter(n => n.notification_type === 'follow').length,
  ...
  const getFilteredNotifications = () => {
    ...
    if (activeTab === 'General') return notifications.filter(n => n.notification_type === 'comment' || n.notification_type === 'like');
    if (activeTab === 'Followers') return notifications.filter(n => n.notification_type === 'follow');
    return notifications;
  };

  const handleNotificationAction = async (notificationId, action) => {
    try {
      const res = await api.post(`posts/notification-actions/${notificationId}/`, { action });
  ...
  const getNotificationText = (notification) => {
    switch (notification.notification_type) {
      case 'like': return `liked your post "${notification.post_title}"`;
      case 'comment': return `commented on your post "${notification.post_title}"`;
      ...
      <div className="px-4">
        {getFilteredNotifications().length === 0 ? (
          <div className="text-center py-20">
            ...
            <h2 className="text-2xl font-bold text-gray-700 dark:text-slate-300 mb-2">Nothing to see here — yet</h2>
            <p className="text-gray-500 dark:text-slate-500">
              From likes to comment and a whole lot more, this is where all the action happens.
            </p>
          </div>
        ) : (
            ...
            {getFilteredNotifications().map((notification) => (
              <div
                ...
                className={`flex items-start gap-3 p-4 mt-2 border-b group hover:bg-gray-100/50 dark:hover:bg-slate-800/50 transition-colors ${
                  !notification.is_read ? 'bg-purple-50 dark:bg-purple-500/5' : ''
